...
export default function Component() {
  ...
  useEffect(() => {
    ...
    function createTextImage() {
      ...
      const moneyTextWidth = logoHeight * 3 // Approximate width for "$money" text

      ctx.translate(canvas.width / 2 - moneyTextWidth / 2, canvas.height / 2 - logoHeight / 2)

      // Draw $money text
      ctx.font = `bold ${logoHeight}px sans-serif`
      ctx.textAlign = "center"
      ctx.textBaseline = "middle"
      ctx.fillText("$money", moneyTextWidth / 2, logoHeight / 2)
      ...
      return logoHeight / 120 // Return scale factor
    }
      ...
      for (let attempt = 0; attempt < 100; attempt++) {
        ...
        if (data[(y * canvas.width + x) * 4 + 3] > 128) {
          return {
            ...
            color: "white",
            scatteredColor: "#30D5C8", // Turquoise green
            isMoneyText: true,
  ...
  return (
    <div className="relative w-full h-dvh flex flex-col items-center justify-center bg-black">
      <canvas
        ...
        aria-label="Interactive particle effect with $money logo"
      />
      <div className="absolute bottom-[100px] text-center z-10">
        <p className="font-mono text-gray-400 text-xs sm:text-base md:text-sm">
          ...
          <a
            ...
            target="_blank"
            className="text-gray-300 hover:text-[#30D5C8] transition-colors duration-300"
            rel="noreferrer"
          >
            hot tokens frame
          </a>
